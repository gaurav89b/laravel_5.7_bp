...
trait FileUploadTrait
{
    ...
    public function validateFile($request, $isDoc = false) {
        $aResponse = array('status' => true, 'error' => "");
        foreach ($request->all() as $key => $value) {
            if ($request->hasFile($key)) {
                $requireString = 'required|mimes:png,gif,jpeg,jpg';
                if ($isDoc) {
                    $requireString = 'required|mimes:png,gif,jpeg,jpg,txt,pdf,doc,docx';
                }
                //$rules = array('file' => 'required|mimes:png,gif,jpeg,jpg'); //'required|mimes:png,gif,jpeg,txt,pdf,doc'
                $rules = array('file' => $requireString);
