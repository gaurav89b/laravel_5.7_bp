...
class Response {
    ...
    public function __construct(Request $request) {
        $debugMode                  = $request->header('DEBUG-MODE', false);
    ...
    public function prepareServiceResponse() {
        ...
        if (!empty($response['messages'])) {
            $aFirstErrorMessage     = array_first($response['messages'][0]);
            $response['message']    = (!empty($aFirstErrorMessage[0])) ? $aFirstErrorMessage[0] :  $response['message'];
        }
